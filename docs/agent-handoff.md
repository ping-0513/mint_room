...
- `docs/gentle-news-design.md` (2026-07-20, Japanese): ideas-only design for a news feature for a news-avoidant user — two lanes (registered interests like LLM releases/cybercrime/Kimetsu/cosmetics + "everyone should know" by life-impact, not sensationalism), an LLM-based gentle filter that blocks violent-crime detail by default, softened opt-in notices for major dark events, RSS-only sources with server-side cache, text-only cards (no images). NOTE: Stage N2 introduces a NEW network boundary (external RSS fetch) and likely one XML-parser dependency — both need master approval before implementation.
