...
- `server/openai.test.mjs`, `server/news.test.mjs`, `public/costs.test.mjs`, `public/backup.test.mjs`, `public/settings.test.mjs` — unit/source regression tests (`npm test`).
- `AGENTS.md`, `docs/collaboration-protocol.md`, `docs/tasks/` — agent rules and the Claude×Codex ticket workflow.
- `public/index.html` — tab shell (Chat / Life / Calendar / Images / Search / Settings), settings, cost-dashboard and backup/restore markup.
- `public/styles.css` — pastel mint theme; light/dark via CSS variables + `[data-theme="dark"]`; system preference respected.
- `public/app.js` — chat state machine, settings binding + localStorage, guarded backup/restore UI, cost recording/rendering, life lists, calendar grid, tabs.
...
- `public/backup.js` — versioned raw backup format, schema preview, future-key preservation, corrupt-category/cost recovery planning, stale-preview detection, verified writes and best-effort rollback.
...
- **Local backup/restore (ticket002, in REVIEW):** Settings exports every `mintroom.*` raw value plus absence markers to versioned JSON after verifying that the same importer accepts it. Import validates all entries before writes, previews counts without message bodies, preserves conflicting future keys, skips a corrupt known category only with consent, and repairs or explicitly resets corrupt cost history. It rechecks the full namespace, starts a current emergency-backup download with a second confirmation, blocks while paid operations are in flight, verifies each write, rolls back without pre-deleting original values, and reloads immediately. Import remains a localStorage operation; close other mint room tabs first as the UI states.
...
- CI + handoff sync 2026-07-21: local in-process suite 88/88 pass; `npm run check` and `git diff --check` pass. Plain local `npm test` is blocked only by this Windows sandbox's child-process `spawn EPERM`; Draft PR #5's ordinary Ubuntu jobs passed on both Node 22 and 24. The workflow uses read-only contents permission, immutable official-Action SHAs, no persisted checkout credentials and a 10-minute timeout.
- Backup/restore 2026-07-21: final local in-process suite 117/117 pass; `npm run check` and `git diff --check` pass. 28 backup-specific tests cover a byte-exact populated six-schema roundtrip, secret namespace exclusion, absent markers, malformed/foreign/oversized input, corrupt-category skip, valid cost-event salvage plus a later append, duplicate rejection, import-time cost reset, future-key collision preservation, stale previews, cost remove→set ordering, quota-aware rollback ordering/retry and permanent rollback failure without broad deletion. Source regression locks pre-download validation, active-AI blocking, post-import stale-save blocking, immediate reload and failure/cancel focus recovery. Initial in-app UI smoke verified Settings controls, export-start status, content-free preview, Cancel/no mutation and no console errors; a later attempt could not obtain an in-app Browser binding, so the final two-confirm destructive path and 375px dark layout remain for reviewer smoke.
...
- Not verified: real OpenAI API call (no key in this environment); backupの最終2段階確認から実write・即reloadまでと375pxダーク表示は、安全修正後のブラウザ再確認待ち（初版のExport/preview/Cancelは確認済み、最終版ではin-app Browser bindingを取得できず）。

## Design docs

- `docs/collaboration-protocol.md` + `docs/tasks/` (2026-07-21, **ACTIVE GUIDELINE**): Claude × Codex collaboration and ticket workflow. Tickets003–006 are DONE; tickets001・002・008 are in REVIEW; ticket007 (Skill regex refactor) remains TODO. Ticket002's final destructive browser path still requires reviewer smoke, while Draft PR #5's Node 22/24 CI has passed. Codex must not edit ACTIVE guideline prose directly (only the living parity/smoke records when affected).
...
2. **Ticket002の相手モデルレビュー＋最終手動smoke** — 実装と自動testは完了済み。2段階確認から実write・即reload、破損fixture、375pxダーク表示を確認してレビュー欄へ記入する。
3. **Ticket008の相手モデルレビュー** — Draft PR #5のNode 22/24 CIは成功済み。レビュー後にDONEへ進める。
4. **Ticket007: Skill Pack regex refactor** — behavior-preserving maintainability work before adding more packs.
5. **Browser smoke test** — load the app, click through tabs, send a mock chat, toggle theme. Fix any DOM typos found.
